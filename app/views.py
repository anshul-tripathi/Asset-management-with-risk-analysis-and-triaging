...
@login_required
def index(request):
    return redirect("/app/demo/dashboard")
...
def vulndetail(request,project_id):
    ...
    cur = conn.cursor()
    cur.execute("SELECT name,ip,port,cve,cvss,projected_severity,inherited_severity,description,solution,poc FROM minerva_" +
...
def asset_details(request,project_id):
    ...
    print "[*] Database connected successfully"
    cur = conn.cursor()
    cur.execute("SELECT name,ip,asset_criticality,cve,cvss,inherited_severity,projected_severity FROM minerva_" +
...
def triage(request,project_id):
    ...
    cur.execute("SELECT id,name,cvss,ip,port,scandate,inherited_severity,projected_severity,triaged,asset_criticality from minerva_"
...
def do_triage(request,project_id):
    ...
    for vuln_asset in id.split(","):
        if vuln_asset:
            ...
            cur.execute("UPDATE minerva_" + project_id + " SET triaged = 1, projected_severity='" +
                        str(request.POST.get("severity")) +
                        "', asset_criticality='" + str(request.POST.get("asset")) +
                        "' WHERE name='" + str(name) +
                        "' AND ip='" + str(ip) + "'")
            conn.commit()
    return redirect("/app/"+project_id+"/triage")
...
def update_asset(request,project_id):
    ...
    for asset in id.split(","):
        if asset:
            cur.execute("UPDATE minerva_" + project_id + " SET triaged = 1, asset_criticality='" + str(
                request.POST.get("asset")) + "' WHERE ip='" + str(asset) + "'")
            conn.commit()
            print "Row " + str(asset) + " updated successfully with message " + str(cur.statusmessage)
    return redirect("/app/" + project_id + "/asset_lookup")

@login_required
def create(request):
    return redirect("/app/" + str(request.POST.get("projectname")) + "/dashboard")
...
def dashboard(request,project_id):
    ...
    try:
        ...
        print "[*] Database connected successfully"
        cur = conn.cursor()
        cur.execute("SELECT DISTINCT scandate FROM minerva_" + project_id + " ORDER BY scandate ASC")
        ...
        for scandate in cur.fetchall():
            ...
            cur1.execute("SELECT ip,inherited_severity,projected_severity,asset_criticality,name,asset_criticality from minerva_"
...
def upload(request,project_id):
    ...
    if "complete" in request.POST:
        cur.execute("select exists(select * from information_schema.tables where table_name=%s)", ("minerva_" + project_id,))
        if cur.fetchone()[0]:
            print "[*] Project " + project_id + " already created"
        else:
            cur.execute('''CREATE TABLE minerva_''' + project_id +
                        ''' (ID BIGSERIAL PRIMARY KEY NOT NULL,
                        cve TEXT,
                        cvss FLOAT(1),
                        ip INET,
    ...
    conn.commit()
    return redirect("/app/" + project_id + "/dashboard")
...
def insert_row_into_databse(row, conn, project_id,scanner,mapping):
    ...
    insertintosqlquery = 'INSERT INTO minerva_' \
                         + project_id + ' (' \
                         'cve,' \
                         'cvss,' \
                         'ip,' \
    ...
    # Projected Severity
    try:
        cur.execute("SELECT projected_severity FROM minerva_" + project_id + " WHERE name='" + row[5] + "'")
...
def vuln_lookup(request,project_id):
    ...
    print "[*] Database connected successfully"
    cur = conn.cursor()
    cur.execute("SELECT DISTINCT name,CVSS,inherited_severity,projected_severity FROM minerva_" + project_id)
    cur1 = conn.cursor()
    projected_risk = 0
    vulns = []
    id = 1
    for entries in cur.fetchall():
        ...
        total = 0
        triaged = ''
        cur1.execute("SELECT projected_severity,ip,asset_criticality,triaged FROM minerva_" + project_id + " WHERE name='" + str(entries[0]) + "'")
...
def update_vuln(request,project_id):
    ...
    return redirect("/app/" + project_id + "/vuln_lookup")
...
def asset_lookup(request,project_id):
    ...
    print "[*] Database connected successfully"
    cur = conn.cursor()
    cur.execute("SELECT DISTINCT ip,hostname,asset_criticality,triaged FROM minerva_" + project_id)
    cur1 = conn.cursor()
    ...
    for entries in cur.fetchall():
        ...
        total = 0
        cur1.execute("SELECT projected_severity,ip,asset_criticality,triaged FROM minerva_" + project_id + " WHERE ip='" + str(entries[0]) + "'")
        for entries1 in cur1.fetchall():
            total = total + float(calculateseverity(entries1[0],entries1[1],entries1[2]).split("$")[0])
...
def get_me_connection():
    ...
    conn = psycopg2.connect("dbname='" + settings.MINERVA_DATABASE['dbname'] +
                            "' user='" + settings.MINERVA_DATABASE['user'] +
                            "' host='" + settings.MINERVA_DATABASE['host'] +
                            "' password='" + settings.MINERVA_DATABASE['password'] + "'")
    print "[*] Database connected successfully"
    return conn
...
def get_project_list():
    ...
    conn = get_me_connection()
    cur = conn.cursor()
    cur.execute("SELECT table_name FROM INFORMATION_SCHEMA.TABLES where table_schema = 'public' AND table_name LIKE 'minerva_%';")
    output = cur.fetchall()
    for entry in output:
        project_list.append(str(entry[0]).replace("minerva_","",1))
    return project_list
